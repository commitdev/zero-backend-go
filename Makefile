...
run: create-db-user
	...
	export AWS_ACCESS_KEY_ID=$(shell aws secretsmanager get-secret-value --region ${region} --secret-id=${PROJECT_NAME}-ci-user-shared3-aws-keys${randomSeed} | jq -r '.SecretString'| jq -r .access_key_id)
	export AWS_SECRET_ACCESS_KEY=$(shell aws secretsmanager get-secret-value --region ${region} --secret-id=${PROJECT_NAME}-ci-user-shared3-aws-keys${randomSeed} | jq -r '.SecretString'| jq -r .secret_key)
