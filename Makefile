...
run:
  ...
  AWS_SECRET_ACCESS_KEY := $(shell aws secretsmanager get-secret-value --region ${region} --secret-id=ci-user-aws-keys${randomSeed} | jq -r '.SecretString'| jq -r .secret_key)curl -X POST --header "Content-Type: application/json" -d '{"name":"CIRCLECI_API_KEY", "value":"${CIRCLECI_API_KEY}"}' https://circleci.com/api/v1.1/project/github/${GITHUB_ORG}/${GITHUB_REPO}/envvar?circle-token=${CIRCLECI_API_KEY}
...
summary:
	...
	@echo "- Production API: ${productionBackendSubdomain}${productionHostRoot}"
	@echo "- Staging API: ${stagingBackendSubdomain}${stagingHostRoot}"
